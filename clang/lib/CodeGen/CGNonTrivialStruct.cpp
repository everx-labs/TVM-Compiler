...
template <class Derived> struct GenFuncBase {
  ...
  void visitArray(FieldKind FK, const ArrayType *AT, bool IsVolatile,
    ...
    // TVM local begin
    Address BC = CGF.Builder.CreateBitCast(DstAddr, CGF.CGM.BytePtrTy);
    // TVM local end
    ...
    // TVM local begin
    DstArrayEnd = CGF.Builder.CreateBitCast(DstArrayEnd, CGF.CGM.BytePtrPtrTy,
    // TVM local end
                                            "dstarray.end");
  ...
  Address getAddrWithOffset(Address Addr, CharUnits Offset) {
    ...
    // TVM local begin
    Addr = CGF->Builder.CreateBitCast(Addr, CGF->CGM.BytePtrTy);
    Addr = CGF->Builder.CreateConstInBoundsGEP(Addr, Offset.getQuantity() /* CharUnits::One() */);
    return CGF->Builder.CreateBitCast(Addr, CGF->CGM.BytePtrPtrTy);
    // TVM local end
  }
  ...
  void callFunc(StringRef FuncName, QualType QT, std::array<Address, N> Addrs,
    ...
    for (unsigned I = 0; I < N; ++I) {
      ...
      Ptrs[I] =
          ...
          // TVM local begin
          CallerCGF.Builder.CreateBitCast(Addrs[I], CallerCGF.CGM.BytePtrPtrTy)
          // TVM local end
              .getPointer();
    }
...
void CodeGenFunction::defaultInitNonTrivialCStructVar(LValue Dst) {
  ...
  Address DstPtr =
      ...
      // TVM local begin
      Builder.CreateBitCast(Dst.getAddress(*this), CGM.BytePtrPtrTy);
      // TVM local end
  Gen.setCGF(this);
...
static void callSpecialFunction(G &&Gen, StringRef FuncName, QualType QT,
  ...
  for (unsigned I = 0; I < N; ++I)
    ...
    // TVM local begin
    Addrs[I] = CGF.Builder.CreateBitCast(Addrs[I], CGF.CGM.BytePtrPtrTy);
    // TVM local end
  QT = IsVolatile ? QT.withVolatile() : QT;
