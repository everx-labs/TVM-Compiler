...

  {
    ...
    if (ShouldEmitVFEInfo) {
      llvm::Value *VFPAddr =
          ...
          // TVM local begin
          Builder.CreateGEP(CGF.ByteTy, VTable, VTableOffset);
          // TVM local end
...
void ItaniumCXXABI::emitVirtualObjectDelete(CodeGenFunction &CGF,
  ...
  if (UseGlobalDelete) {
    ...
    CompletePtr =
        ...
        // TVM local begin
    CGF.Builder.CreateInBoundsGEP(CGF.ByteTy, CompletePtr, Offset);
        // TVM local end
...
void ItaniumCXXABI::emitThrow(CodeGenFunction &CGF, const CXXThrowExpr *E) {
  ...
  // TVM local begin
  if (!Dtor)
    Dtor = llvm::Constant::getNullValue(CGM.BytePtrTy);
  // TVM local end
...
static void emitGlobalDtorWithCXAAtExit(CodeGenFunction &CGF,
  ...
  auto AddrInt8PtrTy =
      ...
      // TVM local begin
      AddrAS ? CGF.ByteTy->getPointerTo(AddrAS) : CGF.BytePtrTy;
      // TVM local end
...
#include "clang/AST/TypeNodes.inc"
  ...
  if (!VTable)
    ...
    // TVM local begin
    VTable = CGM.getModule().getOrInsertGlobal(VTableName, CGM.BytePtrTy);
    // TVM local end
