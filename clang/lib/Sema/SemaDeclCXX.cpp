...
Sema::ActOnDecompositionDeclarator(Scope *S, Declarator &D,
  ...
  for (auto &B : D.getDecompositionDeclarator().bindings()) {
    ...
    if (B.BindExisting) {
      ...
      auto *BD =
          BindingDecl::Create(Context, DC, B.NameLoc, B.Name, BindExpr.get());
      Bindings.push_back(BD);
      ParsingInitForAutoVars.insert(BD);
    } else {
      bool ConsiderLinkage = DC->isFunctionOrMethod() &&
  ...
  for (auto *B : Bindings) {
    ...
    if (auto *Existing = B->getBindExisting()) {
      ExprResult Assignment =
          S.BuildBinOp(nullptr, Loc, BO_Assign, Existing, E.get());
...
static bool checkTupleLikeDecomposition(Sema &S,
  ...
  for (auto *B : Bindings) {
   ...
   // TVM local begin
    if (auto *Existing = B->getBindExisting()) {
      ExprResult Assignment =
          S.BuildBinOp(nullptr, Loc, BO_Assign, Existing, Init);
      if (Assignment.isInvalid())
        return true;
...
const CXXRecordDecl *Sema::FindDecomposableBaseClass(SourceLocation Loc,
  ...
  if (RD->hasDirectFields())
    ...
    if (BestPath->Access != AS_public) {
      Diag(Loc, diag::err_decomp_decl_non_public_base) << RD << BaseType;
      for (auto &BS : *BestPath) {
        if (BS.Base->getAccessSpecifier() != AS_public) {
          Diag(BS.Base->getBeginLoc(), diag::note_access_constrained_by_path)
...
static bool checkMemberDecomposition(Sema &S, ArrayRef<BindingDecl*> Bindings,
  ...
  // TVM local begin
  const CXXRecordDecl *RD = S.FindDecomposableBaseClass(
      Src->getLocation(), OrigRD /* RD */, BasePath);
  // TVM local end
