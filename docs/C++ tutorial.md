...
```
git clone https://github.com/tonlabs/TON-Compiler.git ñ-branch master --single-branch TON-Compiler
cd TON-compiler
mkdir build
cd build
...
```
cargo run call ñabi HelloWorld.abi "<raw address>" hello_world "{}" --sign hw.key
```
...
cargo run call ñabi HelloWorld.abi "<raw address>" hello_world "{}" --sign hw.key
cargo run call ñabi HelloWorld.abi "<raw address>" hello_world "{}" --sign auth.key
```
...
handle(message_balance, message_flags).call<&ICallee::method_name>(parametersÖ);
```
