...
namespace {
/// Define undef in sti, stu, stref, stslice.
class TVMDefineUndef final : public FunctionPass {
...
public:
  static char ID;
  explicit TVMDefineUndef() : FunctionPass(ID) {}
};
...
FunctionPass *llvm::createTVMDefineUndef() { return new TVMDefineUndef(); }
...
static Value *definedValue(Instruction &I, Type *Ty) {
  ...
  if (Ty == Type::getTVMCellTy(Ctx)) {
    IRBuilder<> Builder(&I);
    CallInst *Pushnull =
        Builder.CreateIntrinsic(Intrinsic::tvm_pushnull, {}, {});
...
bool TVMDefineUndef::runOnFunction(Function &F) {
  bool Changed = false;
