...
public:
  ...
  gatherBlockQueueInfo(MachineBasicBlock &MBB, MachineRegisterInfo *MRI,
                       LiveIntervals *LIS, const unordered_set<unsigned> &nonQueueSet,
...
public:
  ...
  void getContents(const StackTreeNode *ancestor, vector<unsigned> &regs) const {
    if (ancestor == this)
      return;
    if (parent != ancestor) {
      assert(parent);
...
#define LEVEL_REGION_WEIGHT 1
  ...
  private:
    StackableRegion(StackableRegionKind kind_)
        : kind(kind_), parent(nullptr), stack(nullptr), subregions(),
          queueInfo(), weight(1) {}
  ...
  public:
    ...
    StackableRegion *getBody() const {
      assert(kind == SRSelfLoop || kind == SRWhileLoop ||
             kind == SRNaturalLoop);
      if (kind == SRSelfLoop)
        return subregions[0];
      else
    ...
    for (auto it = node->succ_begin(); it != node->succ_end(); it++) {
      ...
      for (DJNode *target : joinTargets) {
        if (target->getLevel() <= node->getLevel() &&
            find(nodeJoins.begin(), nodeJoins.end(), target) ==
                nodeJoins.end()) {
  ...
  void StackableRegionStructure::calculateQueueInfo(MachineRegisterInfo *MRI, LiveIntervals *LIS,
      const unordered_set<unsigned> &nonQueueSet) {
    // TODO  getRoot().calculateWeights();
    getRoot().calculateQueueInfo(MRI, LIS, nonQueueSet);
  }

	void StackableRegionStructure::atoneQueueInfo() { 
		getRoot().atoneQueueInfo(); 
	}
  ...
  void StackableRegion::createStackTree(StackTreeNode &stack) {
    ...
    case SRSequence: {
        for (unsigned index = 0; index < size(); index++) {
          ...
          elem->createStackTree(*stack.getTree()->createNode(getLocalStack(), &stack));
          if (index < size() - 1) {
  ...
  void
  StackableRegion::getTailBlocks(vector<MachineBasicBlock *> &blocks) const {
    switch (kind) {
    case SRLeaf:
...
//      calculateConditionStack();
  ...
  private:
    ...
    void distributeOutputResults(const LayeredInstructionDAGNode &node,
		...
		// First element of resulting pair - fixup on entrance, second - fixup on exit.
		void calculateFixups(const StackableRegionStructure &SRS, 
  ...
  */
  ...
  void TVMLocalScheduler::schedule(const vector<LayeredInstructionDAGNode *> nodes,
                              StackTreeNode &stack, MachineBasicBlock &outMBB) {
    if (nodes.empty())
      return;
    ...
    for (LayeredInstructionDAGNode *node : nodes) {
      ...
      if (SMI) {
        ...
        //  dbgs() << "Queue before instruction execution :  " <<
        //  queue.toString()
        //  << "\n";
        for (MachineOperand &MO : MI->uses()) {
          queue.dequeue();
        ...
        //  dbgs() << "Queue after instruction execution :  " <<
        //  queue.toString()
        //  << "\n";
...
void TVMLocalScheduler::calculateForkQueueMerge(const StackableRegionStructure &SRS, const StackableRegion& region, 
    vector<unsigned> &merge) {
  assert(region.getMBB()->succ_size() >  1);
  vector<unsigned> &outputQueue = region.getQueueInfo().actualOutputQueue;
  // Simple algorithm, input queues are the same as Fork output queue without stack.
  ...
  for (unsigned reg : outputQueue) {
    ...
    for (MachineBasicBlock *succMBB : region.getMBB()->successors()) {
      ...
      vector<unsigned> &inputQueue = succRegion->getQueueInfo().actualInputQueue;
...
void TVMLocalScheduler::calculateJoinQueueMerge(const StackableRegionStructure &SRS, const StackableRegion &region,
    vector<unsigned> &merge) {
  ...
  copyList(inputQueue, merge);
}

inline static void embedFixups(MachineBasicBlock* MBB, StackFixup fixup, bool exit,
    unordered_map<MachineBasicBlock *, pair<StackFixup, StackFixup>> &fixups) {
  ...
  for (pair<StackFixup::Change, string> change : changes) {
    ...
    else fixupPair.first(change.first);
  }
}
...
void TVMLocalScheduler::calculateFixups(const StackableRegionStructure &SRS,
  ...
  for (RPOTType::rpo_iterator I = RPOT.begin(), E = RPOT.end(); I != E; ++I) {
    ...
    if (MBB->succ_size() > 1) {
      ...
      for (MachineBasicBlock *succ : MBB->successors()) {
        ...
        if (!merge.empty()) {
          StackFixup fixup = BlockQueueInfo::calculateFixup(MF, inputQueue, merge);
      ...
      StackFixup fixup = BlockQueueInfo::calculateFixup(MF, actualOutput, outputQueue);
      ...
      //dbgs() << "Tail Scope : " << scope->toString() << "\n";
        ...
        if (!stackDefined) {
          ...
          stackDefined = true;
				...
				else assert(branchStack == predStack);

				// Fixup on join exit : { branch.actualOutput } -> { merge }
        vector<unsigned> &actualOutput = predRegion->getQueueInfo().actualOutputQueue;
				StackFixup fixup = BlockQueueInfo::calculateFixup(MF, merge, actualOutput);
        embedFixups(pred, fixup, true, fixups);
      }

			// Fixup on join entrance : { branch.stack, merge } -> { join.stack, join.actualInput }
			vector<unsigned> actualOutput;
      copyList(branchStack, actualOutput);
      copyList(merge, actualOutput);
      ...
      copyList(inputQueue, actualInput);
			StackFixup fixup = BlockQueueInfo::calculateFixup(MF, actualInput, actualOutput);
      embedFixups(MBB, fixup, true, fixups);
 		}
  }
}

  MachineInstr *SchedulerUtils::convertToSForm(const MachineInstr &MI, const TargetInstrInfo *TII,
                                               bool reverse) {
  ...
  void BlockQueueInfo::calculateMaximumCommonChain(
      vector<unsigned> const &queue1, vector<unsigned> const &queue2,
      vector<unsigned> &result, unsigned maxPermutations) {
