...
__tvm_builder build_internal_message (MsgAddressInt* dest, unsigned value) {
  return build_internal_message_bounce(dest, value, 0);
}
...
__tvm_builder build_internal_message_bounce(MsgAddressInt *dest, unsigned value,
  ...
  tvm_serialize_MsgAddressInt(builder, dest);
...
__attribute__((overloadable))
void send_raw_message (__tvm_cell cell, int flags) {
  __builtin_tvm_sendrawmsg(cell, flags);
}

__attribute__((overloadable))
void send_raw_message (__tvm_builder builder, int flags) {
  __builtin_tvm_sendrawmsg(__builtin_tvm_endc(builder), flags);
}
...
MsgAddressInt get_sender_address() {
    ...
    return tvm_deserialize_MsgAddressInt(&s);
}
